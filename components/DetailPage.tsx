...
function About(){return <main id="main"><AboutIntro/><section id="about-pillars" className="export-section"><div className="kx-container"><SectionHead kicker="What sets us apart" title="Domain depth, compounding by design."/><ProofGrid items={aboutPillars}/></div></section><section className="export-section alternate"><div className="kx-container roadmap"><SectionHead kicker="Our three-phase roadmap" title="From deployments to proprietary IP."/><div className="roadmap-track">{phases.map(x=><article key={x.num}><i/><Kicker>{x.num}</Kicker><h3>{x.title}</h3><p>{x.body}</p></article>)}</div></div></section><section className="export-section"><div className="kx-container brand-grid"><div><Image src="/kaarnex/logo.png" alt="Kaarnex logo" width={430} height={115}/></div><article><Kicker>Brand identity</Kicker><h2>The K is a GPS pin.</h2><p>The Kaarnex logo embeds a ‘K’ that forms a GPS location pin—a visual statement of identity: we track, we position, we make vehicles intelligent. The tagline ‘Where Vehicles Meet Intelligence’ defines both the fleet product and the engineering mission.</p></article></div></section><CTA title="Work with a partner that lives in your domain." label="Start the Conversation"/></main>}
